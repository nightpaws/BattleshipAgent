...
import com.innovation.battleships.player.SecondAttempt.boardStates;

public class ImprovedAircraftCarrierDetection implements Player {
...
// Store all attack moves made? - (Miss = M | Hit = H)
	boardStates[][] opponentGrid = new boardStates[12][12];
...
//keep track of how long it takes a ship to be sunk by random positioning
	 ...
	 * Constructor
	 */
	public ImprovedAircraftCarrierDetection() {
	...
	private void addPossibilities(Set<Shot> nextShots,Target target){
		...
		case Destroyer:
			...
			for(Point startingPoint:hitSet){
				for(Shot possibleShot:nextShots){
					if((Math.abs(startingPoint.x-possibleShot.getX())==1&&Math.abs(startingPoint.y-possibleShot.getY())==0)||(Math.abs(startingPoint.y-possibleShot.getY())==1&&Math.abs(startingPoint.x-possibleShot.getX())==0)){
		...
		case Cruiser:
			if(target.isVertical()){
				for(Point startingPoint:hitSet){
					for(Shot possibleShot:nextShots){
						if((Math.abs(startingPoint.x-possibleShot.getX())==0&&Math.abs(startingPoint.y-possibleShot.getY())==1)){
							//Know possible shots is only where you might want to shoot so if one up
				...
				for(Point startingPoint:hitSet){
					for(Shot possibleShot:nextShots){
						if((Math.abs(startingPoint.x-possibleShot.getX())==1&&Math.abs(startingPoint.y-possibleShot.getY())==0)){
							//Know possible shots is only where you might want to shoot so if one left
				...
				case 1:
					...
					for(Shot possibleShot:nextShots){
						for(Point current:hitSet){
							if(Math.abs(possibleShot.getX()-current.x)<2&&Math.abs(possibleShot.getY()-current.y)<2){
								possibleShot.possibleUse();
							}
						}
					}
		...
		case AircraftCarrier:
			...
			case 1:
				...
				break;
			case 2:
				if(hitSet.numberOfEachX().size()==2){
					int yAimFor=-1;
					for(Point startingPoint:hitSet){
						yAimFor=startingPoint.y;
					}
					for(Shot possibleShot:nextShots){
						if(possibleShot.getY()==yAimFor){
							possibleShot.possibleUse();
						}
					}
				}else if(hitSet.numberOfEachX().size()==2){
					int xAimFor=-1;
					for(Point startingPoint:hitSet){
						xAimFor=startingPoint.x;
					}
					for(Shot possibleShot:nextShots){
						if(possibleShot.getX()==xAimFor){
							possibleShot.possibleUse();
						}
					}
				}
				break;
			case 3:
				if(hitSet.numberOfEachX().size()==3){
					//all in a row on x
					int allOnSameY=hitSet.numberOfEachY().get(0)[0];
					int minX=1000;
					int maxX=-1;
					for(Point current:hitSet){
						if(current.x<minX){
					...
					List<Shot> usefulShots=new ArrayList<Shot>();
					usefulShots.add(new Shot(minX-1,allOnSameY));
					usefulShots.add(new Shot(maxX+1,allOnSameY));
					usefulShots.add(new Shot(minX+1,allOnSameY+1));
					usefulShots.add(new Shot(minX+1,allOnSameY-1));
					for(Shot current:nextShots){
						for(Shot useful:usefulShots){
							if(current.equals(useful)){
								current.possibleUse();
							}
						}
					}
				}else if(hitSet.numberOfEachY().size()==3){
					//all in a row on 
					int allOnSameX=hitSet.numberOfEachX().get(0)[0];
					int minY=1000;
					int maxY=-1;
					for(Point current:hitSet){
						if(current.y<minY){
					...
					List<Shot> usefulShots=new ArrayList<Shot>();
					usefulShots.add(new Shot(allOnSameX,minY-1));
					usefulShots.add(new Shot(allOnSameX,maxY+1));
					usefulShots.add(new Shot(allOnSameX+1,minY+1));
					usefulShots.add(new Shot(allOnSameX-1,minY+1));
					for(Shot current:nextShots){
						for(Shot useful:usefulShots){
							if(current.equals(useful)){
					...
					//Centre will be where the most occurrences of
					//y is and the most occurrences of x is.
					int centreX=-1;
					int centreY=-1;
					...
					List<Shot> usefulShots=new ArrayList<Shot>();
					usefulShots.add(new Shot(centreX+1,centreY));
					usefulShots.add(new Shot(centreX-1,centreY));
					usefulShots.add(new Shot(centreX,centreY+1));
					usefulShots.add(new Shot(centreX,centreY-1));
					for(Shot current:nextShots){
						for(Shot useful:usefulShots){
							if(current.equals(useful)){
				...
				break;
			case 4:
				if(hitSet.numberOfEachX().size()==4){
					int allOnSameY=hitSet.numberOfEachY().get(0)[0];
					int minX=1000;
					int maxX=-1;
					for(Point current:hitSet){
					...
					List<Shot> usefulShots=new ArrayList<Shot>();
					usefulShots.add(new Shot(minX,allOnSameY+1));
					usefulShots.add(new Shot(minX,allOnSameY-1));
					usefulShots.add(new Shot(maxX,allOnSameY+1));
					usefulShots.add(new Shot(maxX,allOnSameY-1));
					for(Shot current:nextShots){
						for(Shot useful:usefulShots){
							if(current.equals(useful)){
				...
				}else if(hitSet.numberOfEachY().size()==4){
					int allOnSameX=hitSet.numberOfEachX().get(0)[0];
					int minY=1000;
					int maxY=-1;
					for(Point current:hitSet){
					...
					List<Shot> usefulShots=new ArrayList<Shot>();
					usefulShots.add(new Shot(allOnSameX-1,minY));
					usefulShots.add(new Shot(allOnSameX+1,minY));
					usefulShots.add(new Shot(allOnSameX-1,maxY));
					usefulShots.add(new Shot(allOnSameX+1,maxY));
					for(Shot current:nextShots){
						for(Shot useful:usefulShots){
							if(current.equals(useful)){
	...
	private List<Point> twoTouching(){
		...
		for(Point first:hitSet){
			for(Point second:hitSet){
				if(((first.getX()==second.getX())&&Math.abs(first.getY()-second.getY())==1)||((first.getY()==second.getY())&&Math.abs(first.getX()-second.getX())==1)){
